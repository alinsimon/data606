...
The streak length of 1 means that he got only one hit from that group of shots (streak). In the streak of 1,there is one hit and one miss. For streak length of 0 all the shots were missed.
...
**Insert your answer here**
His typical streak length was 0 with a total of 39 streaks,represented by the red. The longest streak of baskets was 
4; dashed line in blue. 

```{r streak-lenght-distribution}
...
ggplot(data = summarykobe_streak, aes(x = length, y = count_streak)) +
  ...
  geom_point(size =2)+
  geom_text(aes(label =count_streak ),fontface ="bold",nudge_x = 0.07, nudge_y = 0.5)+
...
```{r unfair-coins}
set.seed(100)
sim_unfair_coin_ex3 <- sample(coin_outcomes, 
                              size = 100, 
                              replace = TRUE, 
                              prob = c(0.4,0.6))
result_table <- table(sim_unfair_coin_ex3)
paste(result_table[1], "flips came up heads")
```
...
**Insert your answer here**
45% needs to be added in the vector for the probability of getting Heads; also 133 needs to be used in the parameter size so we create that number of observations.

```{r shooting-percentage-of-45}
set.seed(133)
shot_outcomes <- c("H", "M")
...
**Insert your answer here**
```{r calc_streak-independent-shooter}
...
The typical streak length for this simulated independent shooter with a 45% shooting percentage is  0 , the player's longest streak of baskets in 133 shots is 5.

```{r typical-streak-length}
#grouping by length
summarysim_streak <- sim_streak |>
  group_by(length) |>
  summarise( count_streak = n() )

#sorting streaks per length desc so we can get the max
summarysim_streak_sd <- summarysim_streak|>
  arrange(desc(count_streak))

typical_streak_fst <- summarysim_streak_sd$count_streak[1]
typical_level_fst <- summarysim_streak_sd$length[1]

#sorting length desc so we can get the longest
summarysim_length_sd <- summarysim_streak|>
  arrange(desc(length))

longest_streak_length <- summarysim_length_sd$length[1]
longest_streak <- summarysim_length_sd$count_streak[1]

summarysim_streak

ggplot(data = summarysim_streak, aes(x = length, y = count_streak)) +
  geom_line(color="darkgreen")+
  geom_point(size =2)+
  geom_text(aes(label =count_streak ),fontface ="bold",nudge_x = 0.07, nudge_y = 0.6)+
  geom_vline(xintercept=typical_level_fst, linetype="dashed", color = "red")+
  geom_hline(yintercept=typical_streak_fst, linetype="dashed", color = "red")+
  geom_vline(xintercept=longest_streak_length, linetype="dashed", color = "blue")+
  geom_hline(yintercept=longest_streak, linetype="dashed", color = "blue")+
  xlab('Length')+
  ylab('Number Of Streaks')+
  ggtitle("Streaks and Lengths - Simulated Independent Shooter") +
  theme_minimal()

paste("The typical streak length is ",typical_level_fst, 
      ",the player's longest streak of baskets is",
...
**Insert your answer here**
We will get a similar distribution because when we created the sample, we mirrored the percentage of shooting to 45%.
In other words the sample data set created was done with similar characteristics as Kobe Bryant's dataset.
...
**Insert your answer here**
Simulated shooter (represented by green) got a longest streak of 5 versus Kobe's longest one that was 4 (yellow). For both cases the typical length is 0. After this result there is no evidence that the hot hand model fits Kobe's shooting patterns.

```{r comparison-sim-vs-Kobe}
ggplot(data = summarysim_streak, aes(x = length, y = count_streak)) +
  geom_line(color="darkgreen", size = 1)+
  geom_point(size =2, color="darkgreen")+
  geom_text(aes(label =count_streak ),
            fontface ="bold",nudge_x = 0.07, 
            nudge_y = 0.6)+
  geom_line(data = summarykobe_streak,
            aes(x = length, y = count_streak),
            color ="#FFB90F", 
            size=1 )+
  geom_text(data = summarykobe_streak,
            aes(label =count_streak ),
            fontface ="bold",
            nudge_x = 0.07, 
            nudge_y = 0.6)+
  geom_point(data = summarykobe_streak,
             size =2,
             color="#FFB90F")+
  xlab('Length')+
  ylab('Number Of Streaks')+
  ggtitle("Streaks and Lengths - Simulated Shooter vs Kobe") +
  theme_linedraw()
```
