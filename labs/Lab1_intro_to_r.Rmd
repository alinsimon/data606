...
The direction of the trend line indicates that the number of girls baptized is increasing over the years, the data frame used had already been summarized per year.

```{r scatter_plot_with_trend_line}
ggplot(data = arbuthnot, mapping = aes(x = year, y = girls)) +
  geom_point() +
  geom_smooth( aes(y = girls) , method = "lm" ,se = FALSE) +
...
The proportion of newborns that were boys over the year gradually reduce

```{r prop-boys-overtime}
...
```{r years-included_dimensions-df_variable-names}
#years-included
present$year
...
Present dataset contains a larger number of boys, the range of the minimum and maximum for the number of boys in Present dataset is not included in Arbuthnot's object.
The number of records in Arbuthnot dataset is higher than the one for Present.

```{r comparison_dataset_arbuthnot-present}

summarize_arbuthnot <- arbuthnot %>%
  summarize(min = min(boys), max = max(boys), total_boys=sum(boys), rows = n())
...
No, the direction of the trend line indicates that the proportion of boys being born is decreasing over the years.

```{r observation_arbuthnot_and_present}
#Calculate total
present <- present %>%
  mutate(total = boys + girls)
...
```{r most-total-number-of-births}
#Calculate total
present <- present %>%
  mutate(total = boys + girls)

#sorting the rows
sortdesc_present <- present %>%
  arrange(desc(total))

#getting specific variable and row number
sortdesc_present$year[1]
